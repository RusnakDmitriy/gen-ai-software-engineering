...
Runs the bug-fix agents in order: Research Verifier → Planner → Fixer → Security Verifier → Unit Test Generator. Each stage loads its agent definition from `agents/*.agent.md` and related skills from `.cursor/skills/`.

### Prerequisites

1. **Cursor Agent CLI** (headless `agent` command):
...
3. **Research input** — `research/codebase-research.md` must exist (from the Bug Researcher step).
...
```bash
# Validate config and print what would run (no API calls)
npm run pipeline:dry-run

# Resume from stage 3 (e.g. after planner already finished)
npm run pipeline -- --from 3

# List models available to the CLI
...
| 2 | `planner` | `research/implementation-plan.md` |
| 3 | `bug-fixer` | `research/fix-summary.md` + code fixes |
| 4 | `security-verifier` | `research/security-report.md` |
| 5 | `unit-test-generator` | `research/test-report.md` + unit tests |
